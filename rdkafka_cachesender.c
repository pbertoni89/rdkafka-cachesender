...
#define DEFAULT_LINE_LENGTH 100
    ...
    if(lines_to_send_string != NULL) {
        ...
        if(lines_to_send <= 0) {
            fprintf(stderr, "Invalid lines to send number\n");
            exit(-1);
        }
    }

    if(filename_string == NULL) {
        fprintf(stderr, "A file name should be specified\n");
        exit(-1);
    }

#define DEFAULT_CHUNK_SIZE 16
    char *memory = malloc(DEFAULT_CHUNK_SIZE);
    if(!memory) {
    ...
    int *offset_table = malloc(DEFAULT_CHUNK_SIZE);
    if(!offset_table) {
        fprintf(stderr, "Cannot allocate memory\n");
        exit(-1);
    }
    ...
    for(kk = 0; kk < file_size; kk++) {
        if(kk - last_line_offset >= line_length) {
            if(last_space_offset == last_line_offset) {
                fprintf(stderr, "Line too long, try increasing line length\n");
                exit(-1);
            }
            ...
            if(curr_line * sizeof(int) >= line_mem_size) {
                ...
                offset_table = realloc(offset_table, line_mem_size);
                if(!offset_table) {
                    fprintf(stderr, "Cannot reallocate memory\n");
                    exit(-1);
                }
    ...
    while (run) {
        ...
        for(kk = 0; kk < offset_table_size; kk += lines_to_send) {
            ...
            if(kk > offset_table_size - lines_to_send)
                tosend = offset_table[offset_table_size - 1] - line_offset;
            else
                tosend = offset_table[kk + lines_to_send - 1] - line_offset;
            /*
